		...
		foreach($bulletins as $bulletin) { 
			$nav .= '<li' . ($count === 0 ? " class=\"active\"" : "") . '>
				<a href="#bulletin-' . date( "Y-m-d", strtotime($bulletin['date']) ) . '">
					<time class="dpi-bulletins-time" datetime="' . date( "Y-m-d", strtotime($bulletin['date']) ) . '">' . $Controller->translateDate( $bulletin['date'] ) . '</time>
					<span class="dpi-bulletins-meta">Boletín ' . ($count === 0 ? "actual" : "archivado") . '</span>
				</a>
			</li>';
				...
				<div id="bulletin-' . date( "Y-m-d", strtotime($bulletin['date']) ) . '" class="tab-pane fade in ' . ($count === 0 ? "active" : "") . '">
					<div class="dpi-bulletins-tab">
						...
						<a class="dpi-bulletins-tab--link" href="https://discovermass.com" target="_blank">
							...
							Ver más boletines sobre DiscoverMass
						</a>
					</div>
				<a class="dpi-bulletins-tab--link" href="' . $bulletin['links']['bulletin'] . '"' . ($bulletin['new_tab'] ? " target=\"_blank\"" : "") . '><img class="dpi-bulletins-tab--image" src="' . $bulletin['links']['cover'] . '" alt="portada del boletín"></a>
